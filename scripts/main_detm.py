...
adj_mat = np.load(os.path.join(args.data_path, "adj_mat.npy"))
adj_mat = torch.from_numpy(adj_mat).float().to(device)
...
model = ETM(args.num_topics, args.num_times, args.vocab_size,
            ...
            args.delta, args.nlayer, adj_mat, args.lambda_adj, embeddings, args.train_embeddings,
...
for idx, (sample_batch, index) in enumerate(MyDataloader):
    ...
    saved_z = os.path.join(saved_folder, f"z{idx}.npy")
