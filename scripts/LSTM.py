...
parser.add_argument('--data_path', type=str, default='LSTM_data', help='directory containing data')
...
def train(model, epoch, optimizer):
    ...
    label_filename = os.path.join(args.data_path, "label_train.npy")
    mask_filename = os.path.join(args.data_path, "mask_train.npy")
...
def validate(model):
    ...
    with torch.no_grad():
        ...
        label_filename = os.path.join(args.data_path, "label_test.npy")
        mask_filename = os.path.join(args.data_path, "mask_test.npy")
