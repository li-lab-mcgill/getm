...
parser.add_argument('--train_embeddings1', type=int, default=1, help='whether to include pretrained embedding')
parser.add_argument('--rho_fixed1', type=int, default=0, help='whether to fix rho or train it')
parser.add_argument('--train_embeddings2', type=int, default=1, help='whether to include pretrained embedding')
parser.add_argument('--rho_fixed2', type=int, default=0, help='whether to fix rho or train it')
    ...
    with torch.no_grad():
        ...
        for idx, (sample_batch, index) in enumerate(TestDataloader):
            ...
            if args.bow_norm:
                ...
                normalized_data_batch = data_batch
            theta, _, x = m.get_theta(normalized_data_batch[:, -args.vocab_size2:])
            if args.e2e:
...
for idx, (sample_batch, index) in enumerate(MyDataloader):
    ...
    # data_batch = torch.transpose(data_batch_t, 0, 1).reshape(data_batch_t.size(0) * data_batch_t.size(1),
    #                                                          data_batch_t.size(2))
    theta, _, mu_theta= model.get_theta(data_batch[:, -args.vocab_size2:])
